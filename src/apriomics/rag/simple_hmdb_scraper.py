...
class MetaboliteContext:
    ...
    def to_context_string(self) -> str:
        ...
        context_parts = [f"Metabolite: {self.name} (HMDB ID: {self.hmdb_id})"]
        ...
        if self.pathways:
            context_parts.append(
                f"Pathways: {'; '.join(self.pathways[:10])}"
            )  # Limit to 10

        if self.diseases:
            context_parts.append(
                f"Disease associations: {'; '.join(self.diseases[:8])}"
            )  # Limit to 8
...
class SimpleHMDBScraper:
    ...
    def extract_metabolite_contexts(
        self, max_metabolites: Optional[int] = None
    ) -> Iterator[MetaboliteContext]:
        """
            ...
            if event == "end" and elem.tag == f"{self.namespace}metabolite":
                try:
                    ...
                    if metabolite_count % 1000 == 0:
                        print(
                            f"Processed {metabolite_count} metabolites", file=sys.stderr
                        )
    ...
    def _extract_metabolite_context(
        self, metabolite_elem
    ) -> Optional[MetaboliteContext]:
    ...
    def get_metabolite_by_name(
        self, metabolite_name: str
    ) -> Optional[MetaboliteContext]:
        """
    ...
    def get_metabolites_by_hmdb_ids(
        self, hmdb_ids: list
    ) -> Dict[str, Optional[MetaboliteContext]]:
        """
        ...
        for event, elem in context:
            if event == "end" and elem.tag == f"{self.namespace}metabolite":
                try:
                    ...
                    if processed % 5000 == 0:
                        print(
                            f"Processed {processed} metabolites, {len(remaining_ids)} remaining",
                            file=sys.stderr,
                        )
