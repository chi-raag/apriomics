...
class HMDBVectorBuilder:
    ...
    def __init__(
        ...
        if platform.system() == "Darwin":
            ...
            self.embedding_model.encode = self._safe_encode_wrapper(
        ...
        print(
            f"Embedding dimension: {self.native_dim} -> {self.final_dim}",
            file=sys.stderr,
        )
    ...
    def build_index_from_xml(
        self,
        xml_path: Path,
        output_dir: Path,
        max_metabolites: Optional[int] = None,
        batch_size: int = 1000,
        use_focused_parser: bool = False,
    ) -> Path:
        """
        ...
        print("Building HMDB vector index...", file=sys.stderr)
        ...
        if use_focused_parser:
            ...
            print("Using focused parser (biological relevance only)", file=sys.stderr)
        else:
            parser = HMDBParser(xml_path)
            print("Using standard parser (all HMDB data)", file=sys.stderr)
    ...
    def build_index_from_chunks(
        self, chunks: List[MetaboliteChunk], batch_size: int = 1000
    ):
        """
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            print(
                f"Generated {total_vectors} embeddings in {len(embedding_files)} files",
                file=sys.stderr,
            )
            ...
            if self.embedding_dim and self.embedding_dim < self.native_dim:
                ...
                embedding_dim = self._compute_pca_streaming(
                    embedding_files, total_vectors
                )

            # Phase 3: Build FAISS index incrementally
            print("Phase 3: Building FAISS index...", file=sys.stderr)
            self._build_faiss_index_streaming(
                embedding_files, total_vectors, embedding_dim
            )

        print(
            f"Streaming index built with {self.index.ntotal} vectors", file=sys.stderr
        )

    def _compute_pca_streaming(
        self, embedding_files: List[Path], total_vectors: int
    ) -> int:
        ...
        print(
            f"Computing PCA: {self.native_dim} -> {self.embedding_dim}", file=sys.stderr
        )
        ...
        print(
            f"PCA explained variance ratio: {ipca.explained_variance_ratio_.sum():.3f}",
            file=sys.stderr,
        )
        return self.embedding_dim

    def _build_faiss_index_streaming(
        self, embedding_files: List[Path], total_vectors: int, dim: int
    ):
    ...
    def _apply_pca(self, embeddings: np.ndarray) -> np.ndarray:
        ...
        print(
            f"Applying PCA: {embeddings.shape[1]} -> {self.embedding_dim}",
            file=sys.stderr,
        )
    ...
    def load_index(cls, index_dir: Path) -> "HMDBVectorBuilder":
        ...
        instance = cls(
            ...
            if config["final_dim"] != config["native_dim"]
            else None,
        )
        ...
        print(
            f"  - Dimensions: {config['native_dim']} -> {config['final_dim']}",
            file=sys.stderr,
        )
...
def build_hmdb_index_cli():
    ...
    parser = argparse.ArgumentParser(
        description="Build HMDB vector index from XML dump"
    )
    parser.add_argument("xml_path", help="Path to HMDB XML file")
    parser.add_argument("output_dir", help="Output directory for index")
    parser.add_argument(
        "--max-metabolites", type=int, help="Limit number of metabolites (for testing)"
    )
    parser.add_argument(
        "--embedding-model",
    ...
    parser.add_argument(
        "--device", default="cpu", help="Device for embeddings (cpu/cuda)"
    )
    parser.add_argument(
        "--focused",
        action="store_true",
        help="Use focused parser (biological info only)",
    )
