...
from ..utils.macos_fixes import capture_warnings
...
class HMDBRetriever:
    ...
    def get_metabolite_context(
        self, metabolite_name: str, condition: Optional[str] = None, max_chunks: int = 5
    ) -> str:
        """
    ...
    def get_metabolite_contexts_batch(
        self,
        metabolite_names: List[str],
        condition: Optional[str] = None,
        max_chunks_per_metabolite: int = 3,
    ) -> Dict[str, str]:
        """
    ...
    def search_by_condition(
        self, condition: str, k: int = 20, chunk_types: Optional[List[str]] = None
    ) -> List[RetrievalResult]:
        """
        ...
        """
        return self.search(
            query=condition, k=k, chunk_types=chunk_types or ["diseases", "pathways"]
        )

    def get_similar_metabolites(
        self, metabolite_name: str, k: int = 10
        ...
        results = self.search(query=metabolite_name, k=k * 2, chunk_types=["basic"])
        ...
        for result in results:
            ...
            if name.lower() != metabolite_name.lower():  # Exclude self
                if (
                    name not in metabolite_scores
                    or result.score > metabolite_scores[name]
                ):
                    metabolite_scores[name] = result.score

        # Sort by score and return top k
        similar_metabolites = sorted(
            metabolite_scores.items(), key=lambda x: x[1], reverse=True
        )[:k]
