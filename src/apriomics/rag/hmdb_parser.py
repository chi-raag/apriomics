...
class MetaboliteChunk:
    ...
    def __post_init__(self):
        ...
        if not self.content or not self.content.strip():
            raise ValueError(
                f"Empty content for chunk {self.hmdb_id}:{self.chunk_type}"
            )
...
class HMDBParser:
    ...
    def parse_metabolites(
            ...
            if event == "end" and elem.tag == f"{self.namespace}metabolite":
                try:
                    ...
                    if metabolite_count % 1000 == 0:
                        print(
                            f"Processed {metabolite_count} metabolites, {processed_count} chunks",
                            file=sys.stderr,
                        )
        ...
        print(
            f"Finished parsing. Total: {metabolite_count} metabolites, {processed_count} chunks",
            file=sys.stderr,
        )
...
{"; ".join(pathways)}""".strip()
            ...
            chunks.append(
                MetaboliteChunk(
                    ...
                    metadata={"name": name, "pathways": pathways},
                )
            )
...
{"; ".join(diseases)}""".strip()
            ...
            chunks.append(
                MetaboliteChunk(
                    ...
                    metadata={"name": name, "diseases": diseases},
                )
            )
        ...
        if concentrations:
            ...
            for conc in concentrations[:5]:  # Limit to top 5 examples
                if conc.get("biospecimen") and conc.get("value") and conc.get("units"):
                    conc_examples.append(
                        f"{conc['biospecimen']}: {conc['value']} {conc['units']}"
                    )
...
{"; ".join(conc_examples)}""".strip()
                ...
                chunks.append(
                    MetaboliteChunk(
                        ...
                        metadata={"name": name, "concentrations": concentrations},
                    )
                )
...
{"; ".join(functions)}""".strip()
            ...
            chunks.append(
                MetaboliteChunk(
                    ...
                    metadata={"name": name, "functions": functions},
                )
            )
...
{"; ".join(cellular_processes)}""".strip()
            ...
            chunks.append(
                MetaboliteChunk(
                    ...
                    metadata={"name": name, "cellular_processes": cellular_processes},
                )
            )
    ...
    def _extract_concentrations(self, metabolite_elem) -> List[Dict[str, str]]:
        ...
        concentrations = []
        concentrations_elem = metabolite_elem.find(
            f"{self.namespace}normal_concentrations"
        )
