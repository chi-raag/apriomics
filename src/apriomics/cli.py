...
def build_hmdb_index():
    ...
    parser.add_argument(
        "xml_path", help="Path to HMDB XML dump file (hmdb_metabolites.xml)"
    )

    parser.add_argument("output_dir", help="Output directory for vector index")
    ...
    parser.add_argument(
        "--embedding-dim", type=int, help="PCA dimension reduction (e.g., 384)"
    )
...
def search_hmdb():
    """Search HMDB index."""
    parser = argparse.ArgumentParser(
        description="Search HMDB metabolite database", prog="apriomics search"
    )

    parser.add_argument("index_dir", help="Path to HMDB index directory")

    parser.add_argument("query", help="Search query (metabolite name, condition, etc.)")
...
def show_stats():
    ...
    parser = argparse.ArgumentParser(
        description="Show HMDB index statistics", prog="apriomics stats"
    )
...
def main():
    ...
    build_parser = subparsers.add_parser(
        "build-index", help="Build HMDB vector index from XML dump"
    )

    # Search command
    search_parser = subparsers.add_parser(
        "search", help="Search HMDB metabolite database"
    )
