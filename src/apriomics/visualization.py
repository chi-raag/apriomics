...
class MarkovFieldVisualizer:
    ...
    def plot_network(self, 
        ...
        positive_edges = [(u, v) for u, v, d in self.graph.edges(data=True) if d['sign'] > 0]
        negative_edges = [(u, v) for u, v, d in self.graph.edges(data=True) if d['sign'] < 0]
        ...
        legend_elements = [
            ...
            plt.Line2D([0], [0], color='red', lw=2, linestyle='--', label='Substrate-Product (-1)')
        ]
        ax.legend(handles=legend_elements, loc='upper right')
        
        ax.set_title('Signed Metabolic Network\n(Markov Field Prior Structure)', 
    ...
    def plot_laplacian_heatmap(self, 
        ...
        subset_edges = [(m1, m2) for (m1, m2) in self.signed_edges.keys() 
        ...
        display_names = [name[:10] + '...' if len(name) > 10 else name for name in metabolites]
        ...
        ax.set_title('Graph Laplacian Matrix\n(Markov Field Prior Precision)', 
                    fontsize=14, fontweight='bold')
    ...
    def plot_degree_distribution(self, 
        ...
        plt.suptitle('Metabolic Network Degree Analysis', fontsize=14, fontweight='bold')
        plt.tight_layout()
    ...
    def get_network_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'positive_edges': len([(u, v) for u, v, d in self.graph.edges(data=True) if d['sign'] > 0]),
            'negative_edges': len([(u, v) for u, v, d in self.graph.edges(data=True) if d['sign'] < 0]),
...
class LLMPriorVisualizer:
    ...
    def plot_importance_scores(self, 
        ...
        bars = ax.barh(range(len(top_metabolites)), 
        ...
        ax.set_title('Top Metabolites by Bayesian Prior Importance\n(Colored by Expected Direction)', 
                    fontsize=14, fontweight='bold')
        
        # Add legend
        legend_elements = [plt.Rectangle((0,0),1,1, facecolor=color, alpha=0.7, label=direction.title()) 
                          for direction, color in direction_colors.items()]
        ax.legend(handles=legend_elements, loc='lower right')
        
        # Add value labels on bars
        for i, (bar, score) in enumerate(zip(bars, top_metabolites['importance_score'])):
    ...
    def plot_prior_distributions(self, 
        ...
        if metabolites is None:
            ...
            metabolites = self.scores_df.nlargest(12, 'importance_score')['metabolite'].tolist()
        ...
        for i, (_, row) in enumerate(subset_df.iterrows()):
            ...
            x = np.linspace(row['expected_log2fc'] - 3*row['prior_sd'], 
                           row['expected_log2fc'] + 3*row['prior_sd'], 100)
            y = (1/np.sqrt(2*np.pi*row['prior_sd']**2)) * \
                np.exp(-0.5*((x - row['expected_log2fc'])/row['prior_sd'])**2)
            ...
            ax.set_title(f"{row['metabolite'][:15]}\n{row['direction']} ({row['confidence']})", 
                        fontsize=10)
        ...
        plt.suptitle('Prior Distributions for Expected Log2 Fold Changes', 
                    fontsize=14, fontweight='bold')
        plt.tight_layout()
    ...
    def plot_confidence_analysis(self, 
        ...
        ax1.pie(confidence_counts.values, labels=confidence_counts.index, autopct='%1.1f%%',
               colors=['lightgreen', 'yellow', 'lightcoral'])
        ax1.set_title('Confidence Distribution')
        
        # Magnitude vs. Importance scatter
        magnitude_order = ['minimal', 'small', 'moderate', 'large']
        magnitude_numeric = self.scores_df['magnitude'].map({m: i for i, m in enumerate(magnitude_order)})
        
        scatter = ax2.scatter(magnitude_numeric, self.scores_df['importance_score'], 
                            c=self.scores_df['prior_sd'], cmap='viridis', alpha=0.7)
    ...
    def plot_direction_analysis(self, 
                              figsize: Tuple[int, int] = (12, 8),
                              save_path: Optional[str] = None) -> plt.Figure:
        """
        ...
        for i, direction in enumerate(direction_order):
            ...
            if not subset.empty:
                ax2.scatter([i] * len(subset), subset['importance_score'], 
                          alpha=0.6, s=50, label=direction)
    ...
    def create_interactive_explorer(self) -> Any:
        ...
        def update_plot(change):
            with output:
                ...
                row = self.scores_df[self.scores_df['metabolite'] == selected_metabolite].iloc[0]
                ...
                # Metadata display
                ax2.text(0.1, 0.9, f"Importance: {row['importance_score']:.3f}", 
                        transform=ax2.transAxes, fontsize=12)
                ax2.text(0.1, 0.8, f"Direction: {row['direction']}", 
                        transform=ax2.transAxes, fontsize=12)
                ax2.text(0.1, 0.7, f"Magnitude: {row['magnitude']}", 
                        transform=ax2.transAxes, fontsize=12)
                ax2.text(0.1, 0.6, f"Confidence: {row['confidence']}", 
                        transform=ax2.transAxes, fontsize=12)
                ax2.text(0.1, 0.5, f"Expected Log2FC: {row['expected_log2fc']:.3f}", 
                        transform=ax2.transAxes, fontsize=12)
                ax2.text(0.1, 0.4, f"Prior SD: {row['prior_sd']:.3f}", 
                        transform=ax2.transAxes, fontsize=12)
                ax2.set_xlim(0, 1)
                ax2.set_ylim(0, 1)
                ...
                # Rationale
                rationale_text = row['rationale'][:200] + '...' if len(row['rationale']) > 200 else row['rationale']
                ax3.text(0.05, 0.95, 'LLM Rationale:', transform=ax3.transAxes, 
                        fontsize=12, fontweight='bold')
                ax3.text(0.05, 0.05, rationale_text, transform=ax3.transAxes, 
                        fontsize=10, wrap=True, verticalalignment='bottom')
                ax3.set_xlim(0, 1)
                ax3.set_ylim(0, 1)
                ax3.axis('off')
                
                # Comparison with others
                comparison_df = self.scores_df[self.scores_df['direction'] == row['direction']].copy()
                ax4.scatter(comparison_df['expected_log2fc'], comparison_df['importance_score'], 
                          alpha=0.6, s=50, label='Same direction')
                ax4.scatter([row['expected_log2fc']], [row['importance_score']], 
                          color='red', s=100, label='Selected', zorder=5)
    ...
    def get_summary_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'direction_distribution': self.scores_df['direction'].value_counts().to_dict(),
            'confidence_distribution': self.scores_df['confidence'].value_counts().to_dict(),
            'magnitude_distribution': self.scores_df['magnitude'].value_counts().to_dict(),
...
def create_comprehensive_report(markov_visualizer: MarkovFieldVisualizer,
                               bayesian_visualizer: LLMPriorVisualizer,
                               output_dir: str = "prior_analysis_report") -> None:
    """
    ...
    markov_visualizer.plot_laplacian_heatmap(save_path=str(output_path / "laplacian_heatmap.png"))
    
    print("3. Generating degree distribution...")
    markov_visualizer.plot_degree_distribution(save_path=str(output_path / "degree_distribution.png"))
    
    # Bayesian prior visualizations
    print("4. Generating Bayesian importance scores...")
    bayesian_visualizer.plot_importance_scores(save_path=str(output_path / "bayesian_importance_scores.png"))
    
    print("5. Generating prior distributions...")
    bayesian_visualizer.plot_prior_distributions(save_path=str(output_path / "prior_distributions.png"))
    
    print("6. Generating confidence analysis...")
    bayesian_visualizer.plot_confidence_analysis(save_path=str(output_path / "confidence_analysis.png"))
    
    print("7. Generating direction analysis...")
    bayesian_visualizer.plot_direction_analysis(save_path=str(output_path / "direction_analysis.png"))
...
def plot_signed_network(signed_edges: Dict[Tuple[str, str], int], **kwargs) -> plt.Figure:
...
def analyze_priors(signed_edges: Dict[Tuple[str, str], int], 
                  prior_scores: List[Any],
                  output_dir: str = "prior_analysis") -> None:
    """
