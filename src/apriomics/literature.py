...
class DSPyPathwayCleaner:
    ...
    def clean_pathway_list(self, pathway_list: List[str]) -> List[str]:
        ...
        try:
            ...
            # Parse the result
            cleaned_list = [
                line.strip() for line in cleaned_list_text.split("\n") if line.strip()
            ]

            # Sanity check
            if not cleaned_list or not all(len(p) > 3 for p in cleaned_list):
                print(
                    f"Warning: DSPy cleaning returned unexpected result: {cleaned_list_text}"
                )
...
def clean_pathway_list_with_llm(
    ...
    try:
        ...
        if not cleaned_list or not all(len(p) > 3 for p in cleaned_list):
            print(
                f"Warning: LLM cleaning returned unexpected result: {cleaned_list_text}"
            )
