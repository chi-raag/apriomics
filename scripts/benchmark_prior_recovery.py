...
def load_or_generate_quantitative_predictions(
    metabolite_names,
    sample_data,
    use_hmdb_context=True,
    ...
    """Load cached quantitative predictions if available, otherwise generate and cache them."""
    ...
    model_suffix = f"_{model_name.replace('-', '_').replace('.', '_')}"
    temp_suffix = f"_temp{temperature:.1f}".replace(".", "")
    cache_filename = f"quantitative_predictions{dataset_suffix}{context_suffix}{model_suffix}{temp_suffix}_{len(metabolite_names)}metabolites.pkl"
    cache_path = os.path.join(cache_dir, cache_filename)
    ...
    # Try to load from cache first
    if os.path.exists(cache_path):
        print(f"Loading cached quantitative predictions from {cache_path}")
        try:
            with open(cache_path, "rb") as f:
                cached_predictions = pickle.load(f)
    ...
    # Generate fresh quantitative predictions
    print(
        f"Generating fresh quantitative predictions ({'with' if use_hmdb_context else 'without'} HMDB context, {model_name})..."
    )
...
"""
    ...
    quantitative_predictions = get_llm_quantitative_priors(
        priors=priors_data,
        condition=condition,
        use_hmdb_context=use_hmdb_context,
        model_name=model_name,
    ...
    try:
        with open(cache_path, "wb") as f:
            pickle.dump(quantitative_predictions, f)
        print(f"✅ Saved quantitative predictions to cache: {cache_path}")
    except Exception as e:
        print(f"⚠️  Warning: Could not save to cache: {e}")
...
def run_benchmark(
    ...
    Args:
        methods: List of methods to run. If None, runs all methods.
                Available: ['uninformative_bayesian',
                           'oracle_bayesian', '4o_no_context_conservative',
                           '4o_with_context_conservative', 'o3-mini_no_context_conservative',
                           'o3-mini_with_context_conservative', 'o3_no_context_conservative']
    """
    ...
    all_methods = [
        ...
        "oracle_bayesian",
        "4o_no_context_conservative",
        "4o_with_context_conservative",
        "o3-mini_no_context_conservative",
        "o3-mini_with_context_conservative",
        "o3_no_context_conservative",
        "4o_no_context_quantitative",
        "4o_with_context_quantitative",
        "o3-mini_no_context_quantitative",
        "o3-mini_with_context_quantitative",
        "o3_no_context_quantitative",
    ]
    ...
    # Load or generate quantitative predictions once per model/context combo
    print("--- Loading/generating quantitative predictions ---")

    quantitative_predictions = {
        "4o_no_context": load_or_generate_quantitative_predictions(
            metabolite_names,
            sample_data,
            use_hmdb_context=False,
            model_name="gpt-4o-2024-08-06",
        ...
        "4o_with_context": load_or_generate_quantitative_predictions(
            metabolite_names,
            sample_data,
            use_hmdb_context=True,
            model_name="gpt-4o-2024-08-06",
        ...
        "o3-mini_no_context": load_or_generate_quantitative_predictions(
            metabolite_names,
            sample_data,
            use_hmdb_context=False,
            model_name="o3-mini-2025-01-31",
        ...
        "o3-mini_with_context": load_or_generate_quantitative_predictions(
            metabolite_names,
            sample_data,
            use_hmdb_context=True,
            model_name="o3-mini-2025-01-31",
        ...
        "o3_no_context": load_or_generate_quantitative_predictions(
            metabolite_names,
            sample_data,
            use_hmdb_context=False,
            model_name="o3-2025-04-16",
    ...
    priors = {
        ...
        # Direct quantitative predictions (no mapping needed)
        "4o_no_context_quantitative": quantitative_predictions["4o_no_context"],
        "4o_with_context_quantitative": quantitative_predictions["4o_with_context"],
        "o3-mini_no_context_quantitative": quantitative_predictions[
            "o3-mini_no_context"
        ],
        "o3-mini_with_context_quantitative": quantitative_predictions[
            "o3-mini_with_context"
        ],
        "o3_no_context_quantitative": quantitative_predictions["o3_no_context"],
    }
    ...
    for n_per_group in sample_sizes:
        for replicate in range(n_replicates):
            ...
            try:
                ...
                # Test LLM methods (both qualitative and quantitative)
                llm_methods = [
                    ...
                    ("4o_no_context_quantitative", "GPT-4o (No Context, Quantitative)"),
                    (
                        "4o_with_context_quantitative",
                        "GPT-4o (With Context, Quantitative)",
                    ),
                    (
                        "o3-mini_no_context_quantitative",
                        "O3 Mini (No Context, Quantitative)",
                    ),
                    (
                        "o3-mini_with_context_quantitative",
                        "O3 Mini (With Context, Quantitative)",
                    ),
                    ("o3_no_context_quantitative", "O3 (No Context, Quantitative)"),
                ]
    ...
    return pd.DataFrame(results), bias_variance_results, pd.DataFrame(detailed_results)
...
def create_benchmark_visualizations(results_df, ground_truth_lnfc):
    ...
    method_names = {
        ...
        "o3_no_context_conservative": "O3 (No Context, Conservative)",
        "4o_no_context_quantitative": "GPT-4o (No Context, Quantitative)",
        "4o_with_context_quantitative": "GPT-4o (With Context, Quantitative)",
        "o3-mini_no_context_quantitative": "O3 Mini (No Context, Quantitative)",
        "o3-mini_with_context_quantitative": "O3 Mini (With Context, Quantitative)",
        "o3_no_context_quantitative": "O3 (No Context, Quantitative)",
    }
...
def main(
    ...
    Args:
        methods: List of methods to run. If None, runs all methods.
                Available: ['uninformative_bayesian',
                           'oracle_bayesian', '4o_no_context_conservative', '4o_with_context_conservative',
    ...
    results_df, bias_variance_df, detailed_df = run_benchmark(
        abundance_data,
        group_labels,
        metabolite_names,
        sample_data,
    ...
    raw_estimates_filename = f"benchmark_raw_estimates{suffix}.csv"
    ...
    detailed_df.to_csv(f"{output_dir}/{raw_estimates_filename}", index=False)

    print(f"\nResults saved to {output_dir}/")
    print(f"- {results_filename}")
    print(f"- {summary_filename}")
    print(f"- {bias_variance_filename}")
    print(f"- {raw_estimates_filename}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help='Suffix to add to output filenames (e.g., "_full_test")',
    )
    parser.add_argument(
        "--sample-sizes",
        nargs="+",
