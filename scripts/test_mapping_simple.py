...
def test_mapping_variations():
    ...
    for mapping_name, priors in mappings.items():
        ...
        print(f"\n{mapping_name}:")
        print(f"  Mean effects: {np.mean(np.abs(means)):.3f} ± {np.std(np.abs(means)):.3f}")
    ...
    for metabolite in example_metabolites:
        ...
        print(f"\n{metabolite}:")
        print(f"  LLM: {pred.get('prediction', 'N/A')} / {pred.get('magnitude', 'N/A')} / conf={pred.get('confidence', 'N/A'):.2f}")
        
        for mapping_name, priors in mappings.items():
            if metabolite in priors:
                prior = priors[metabolite]
                print(f"  {mapping_name:20s}: μ={prior['expected_log2fc']:+.3f}, σ={prior['prior_sd']:.2f}")
    
    # Key insight
    current_effects = [abs(p['expected_log2fc']) for p in mappings["current_moderate"].values()]
    stronger_effects = [abs(p['expected_log2fc']) for p in mappings["stronger_magnitudes"].values()]
    
    print(f"\n🔍 KEY INSIGHT:")
    print("="*50)
    print(f"Current avg magnitude: {np.mean(current_effects):.3f}")
    print(f"Stronger avg magnitude: {np.mean(stronger_effects):.3f}")
    print(f"Magnitude increase: {np.mean(stronger_effects)/np.mean(current_effects):.1f}x")
