...
def validate_implementation():
    ...
    print(f"   Class distribution: {np.bincount(y)} (Control={np.sum(y==0)}, Case={np.sum(y==1)})")
    ...
    print(f"   Dummy classifier (most frequent): {dummy_scores.mean():.3f} ± {dummy_scores.std():.3f}")
    
    # Dummy classifier (stratified)
    dummy_strat = DummyClassifier(strategy='stratified', random_state=42)
    dummy_strat_scores = cross_val_score(dummy_strat, X, y, cv=5, scoring='accuracy')
    print(f"   Dummy classifier (stratified): {dummy_strat_scores.mean():.3f} ± {dummy_strat_scores.std():.3f}")
    ...
    print(f"   Logistic Regression (default): Acc={lr_scores.mean():.3f}±{lr_scores.std():.3f}, AUC={lr_auc_scores.mean():.3f}±{lr_auc_scores.std():.3f}")
    ...
    print(f"   Logistic Regression (L2, C=1): Acc={lr_l2_scores.mean():.3f}±{lr_l2_scores.std():.3f}, AUC={lr_l2_auc_scores.mean():.3f}±{lr_l2_auc_scores.std():.3f}")
    ...
    lr_strong_auc_scores = cross_val_score(lr_strong, X_scaled, y, cv=5, scoring='roc_auc')
    
    print(f"   Logistic Regression (L2, C=0.1): Acc={lr_strong_scores.mean():.3f}±{lr_strong_scores.std():.3f}, AUC={lr_strong_auc_scores.mean():.3f}±{lr_strong_auc_scores.std():.3f}")
    ...
    print(f"   Coefficient statistics: mean={coef_magnitudes.mean():.3f}, max={coef_magnitudes.max():.3f}")
    print(f"   Large coefficients (>1.0): {np.sum(coef_magnitudes > 1.0)}/{len(coef_magnitudes)}")
    ...
    if lr_scores.mean() > 0.95:
        print("   ⚠️  WARNING: Accuracy > 0.95 suggests potential data leakage or very easy problem")
    elif lr_scores.mean() > 0.85:
        print("   ⚠️  CAUTION: High accuracy - check if this is reasonable for the problem")
    else:
    ...
    print(f"   CV stability across random seeds: {np.mean(cv_results):.3f} ± {cv_stability:.3f}")
...
def check_data_leakage():
    ...
    if len(unique_samples) < X.shape[0]:
        print(f"   ⚠️  WARNING: Found {X.shape[0] - len(unique_samples)} duplicate samples!")
    else:
    ...
    if max_corr > 0.9:
        print("   ⚠️  WARNING: Very high feature-target correlation suggests potential leakage")
    elif max_corr > 0.8:
    ...
    for idx in reversed(top_corr_idx):
        print(f"     {metabolite_names[idx]}: r={np.corrcoef(X_scaled[:, idx], y)[0, 1]:.3f}")
...
if __name__ == "__main__":
    ...
    print(f"Logistic regression: {results['lr_accuracy']:.3f} (AUC: {results['lr_auc']:.3f})")
