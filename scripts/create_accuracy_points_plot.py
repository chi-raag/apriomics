...
def create_accuracy_points_plot():
    ...
    sns.stripplot(
        data=results_df, x="method", y="accuracy", size=8, alpha=0.8, jitter=True, ax=ax
    )
    ...
    ax.text(
        0.02,
        dummy_baseline + 0.005,
        f"Dummy baseline: {dummy_baseline:.3f}",
    ...
    plt.savefig(
        f"{output_dir}/classification_accuracy_points.png", dpi=300, bbox_inches="tight"
    )
    ...
    summary_stats = (
        results_df.groupby("method")["accuracy"]
    ...
    for method in summary_stats.index:
        ...
        print(
            f"{method:<25} {count:<6} {mean_acc:<6.3f} {std_acc:<6.3f} {min_acc:<6.3f} {max_acc:<6.3f}"
        )
