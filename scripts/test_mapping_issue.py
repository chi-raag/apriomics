...
def load_ground_truth():
    ...
    data_path = (
        "/Users/chiraag/Projects/gwu/lab/apriomics/output/mtbls1_hmdb_mapped_data.pkl"
    )
...
def test_mapping_impact():
    ...
    for mapping_name, priors in mappings_to_test.items():
        ...
        if len(prior_means) > 0:
            ...
            rmse = np.sqrt(
                np.mean((np.array(prior_means) - np.array(true_values)) ** 2)
            )
            ...
            print(
                f"{mapping_name:20s}: r={correlation:.3f} (p={p_value:.3f}), RMSE={rmse:.3f}, n={len(prior_means)}"
            )
    ...
    if len(results) >= 2:
        current_best = max(
            [r for r in results if "current" in r["mapping"]],
        ...
        if oracle_result:
            ...
            print(
                f"📈 Oracle RMSE improvement: {current_best['rmse'] - oracle_result['rmse']:.3f}"
            )

            if improvement > 0.1:
                print(
                    "✅ MAPPING IS LIKELY THE ISSUE - Oracle mapping shows substantial improvement!"
                )
                print(
                    "   Recommendation: Redesign the magnitude/confidence → prior mapping"
                )
            elif improvement > 0.05:
                print("⚠️  MAPPING CONTRIBUTES TO ISSUE - Moderate improvement possible")
                print(
                    "   Recommendation: Refine mapping, but also check LLM prediction quality"
                )
            else:
                print(
                    "❌ MAPPING NOT THE MAIN ISSUE - Oracle mapping doesn't help much"
                )
        ...
        for metabolite in example_metabolites:
            ...
            print(f"\n{metabolite}:")
            print(
                f"  LLM: {pred.get('prediction', 'N/A')} / {pred.get('magnitude', 'N/A')} / conf={pred.get('confidence', 'N/A')}"
            )
            ...
            for mapping_name, priors in mappings_to_test.items():
                if metabolite in priors:
                    ...
                    print(
                        f"  {mapping_name}: μ={prior['expected_log2fc']:.3f}, σ={prior['prior_sd']:.3f}"
                    )
