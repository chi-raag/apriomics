...
"""
    ...
    methods = [
        ("LLM-Adaptive-LASSO", lambda X_tr, y_tr, X_te: fit_llm_adaptive_lasso(
            X_tr, y_tr, X_te, metabolite_names, qualitative_predictions)),
        ("LLM-Informed-Bayesian", lambda X_tr, y_tr, X_te: fit_llm_informed_bayesian(
            X_tr, y_tr, X_te, metabolite_names, qualitative_predictions)),
...
def create_predictive_boxplot(results_df):
    ...
    axes[0].set_title("Classification Accuracy by Method", fontsize=14, fontweight='bold')
    ...
    axes[1].set_title("Area Under ROC Curve (AUC) by Method", fontsize=14, fontweight='bold')
    ...
    plt.savefig(f"{output_dir}/predictive_performance_boxplot_with_llm_adaptive_lasso.png", 
                dpi=300, bbox_inches="tight")
    print(f"📊 Boxplot saved to {output_dir}/predictive_performance_boxplot_with_llm_adaptive_lasso.png")
    ...
    for method in acc_summary.index:
        ...
        print(f"  {method:25s}: {mean_acc:.3f} ± {std_acc:.3f} (range: {min_acc:.3f}-{max_acc:.3f})")
    ...
    for method in auc_summary.index:
        ...
        print(f"  {method:25s}: {mean_auc:.3f} ± {std_auc:.3f} (range: {min_auc:.3f}-{max_auc:.3f})")
...
def main():
    ...
    # Save results
    output_dir = "/Users/chiraag/Projects/gwu/lab/apriomics/output"
    results_df.to_csv(f"{output_dir}/predictive_performance_cv_results.csv", index=False)
