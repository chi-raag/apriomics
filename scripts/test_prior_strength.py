...
"""
    ...
    for strength in strengths:
        ...
        numerical_priors = map_qualitative_to_classification_priors(qualitative_predictions, strength)
        
        # Check prior statistics
        prior_means = [numerical_priors[name]["expected_coef"] for name in metabolite_names 
                      if name in numerical_priors]
        prior_sds = [numerical_priors[name]["prior_sd"] for name in metabolite_names 
                    if name in numerical_priors]
        
        print(f"  Prior means: {np.mean(np.abs(prior_means)):.3f} ± {np.std(prior_means):.3f}")
    ...
    if len(results_df.dropna()) > 1:
        ...
        llm_results = results_df[results_df['strength'].isin(['conservative', 'moderate', 'strong'])].dropna()
        if len(llm_results) > 1:
            corr_strength_acc = np.corrcoef(llm_results['mean_abs_prior'], llm_results['accuracy'])[0,1]
            corr_strength_auc = np.corrcoef(llm_results['mean_abs_prior'], llm_results['auc'])[0,1]
...
def fit_llm_bayesian_classifier_strength(X_train, y_train, X_test, metabolite_names, numerical_priors):
    ...
    with pm.Model() as model:
        ...
        beta = pm.Normal("beta", mu=prior_means, sigma=np.minimum(prior_sds, 2.0), shape=n_features)
        ...
        with warnings.catch_warnings():
            ...
            idata = pm.sample(500, tune=500, cores=2, progressbar=False,
