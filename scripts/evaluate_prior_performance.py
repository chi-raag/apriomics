...
def fit_bayesian_model(abundance_data, group_labels, priors_dict, metabolite_names):
    ...
    prior_means = np.array([priors_dict[name]['expected_log2fc'] for name in metabolite_names])
    ...
    with pm.Model() as model:
        ...
        y_obs = pm.Normal(
            ...
            observed=abundance_data.values
        )
...
def cross_validate_priors(abundance_data, group_labels, prior_configs, metabolite_names, n_folds=5):
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kf.split(abundance_data)):
        ...
        for prior_name, priors_dict in prior_configs.items():
            ...
            try:
                ...
                model, idata = fit_bayesian_model(X_train, y_train, priors_dict, metabolite_names)
...
def analyze_results(cv_results):
    ...
    for metric in metrics:
        ...
        for prior_type in prior_types:
            if metric in summary_stats[prior_type]:
                ...
                print(f"{prior_type:20s}: {stats['mean']:.3f} ± {stats['std']:.3f} "
    ...
    for metric in metrics:
        ...
        if all(col in cv_results.columns for col in [uniform_col, llm_no_context_col]):
            stat, pval = ttest_rel(cv_results[llm_no_context_col], cv_results[uniform_col])
            print(f"  LLM (no context) vs Uniform: t={stat:.3f}, p={pval:.4f}")
        
        if all(col in cv_results.columns for col in [uniform_col, llm_with_context_col]):
            stat, pval = ttest_rel(cv_results[llm_with_context_col], cv_results[uniform_col])
            print(f"  LLM (with context) vs Uniform: t={stat:.3f}, p={pval:.4f}")
        
        if all(col in cv_results.columns for col in [llm_no_context_col, llm_with_context_col]):
            stat, pval = ttest_rel(cv_results[llm_with_context_col], cv_results[llm_no_context_col])
            print(f"  LLM (with context) vs LLM (no context): t={stat:.3f}, p={pval:.4f}")
...
def create_visualizations(cv_results):
    ...
    plt.tight_layout()
    plt.savefig('/Users/chiraag/Projects/gwu/lab/apriomics/output/prior_performance_comparison.png', 
    ...
    sns.stripplot(data=accuracy_data, x='prior_type', y='value', 
                  size=8, alpha=0.7, ax=ax)
    sns.boxplot(data=accuracy_data, x='prior_type', y='value', 
                width=0.3, ax=ax, color='lightgray', alpha=0.5)
    ...
    plt.tight_layout()
    plt.savefig('/Users/chiraag/Projects/gwu/lab/apriomics/output/accuracy_detailed_comparison.png', 
                dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    print(f"Dataset loaded: {len(metabolite_names)} metabolites, {len(group_labels)} samples")
