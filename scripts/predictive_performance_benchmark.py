...
def map_qualitative_to_classification_priors(qualitative_predictions, prior_strength="moderate"):
    """
    ...
    for metabolite, qual_pred in qualitative_predictions.items():
        ...
        if prior_strength == "conservative":
            expected_coef, prior_sd = conservative_classification_prior(prediction, confidence)
        elif prior_strength == "moderate":
            expected_coef, prior_sd = moderate_classification_prior(prediction, confidence)
        elif prior_strength == "strong":
            expected_coef, prior_sd = strong_classification_prior(prediction, confidence)
        else:
...
def load_and_prepare_data():
    ...
    print(f"Dataset: {len(abundance_data)} samples, {len(metabolite_names)} metabolites")
    print(f"Class distribution: Control={np.sum(group_labels == 0)}, Case={np.sum(group_labels == 1)}")
    
    return abundance_data, group_labels, metabolite_names, sample_data


def fit_llm_bayesian_classifier(X_train, y_train, X_test, metabolite_names, sample_data):
...
"""
    ...
    try:
        ...
        qualitative_predictions = {name: {
            "prediction": "unchanged", "magnitude": "small", 
            "confidence": 0.5, "reasoning": "Fallback"
        } for name in metabolite_names}
    
    # Map to classification-specific numerical priors
    numerical_priors = map_qualitative_to_classification_priors(qualitative_predictions, "moderate")
    
    # Fit Bayesian logistic regression
    n_features = X_train.shape[1]
    
    # Get prior means and sds (aligned with features)
    prior_means = np.array([numerical_priors.get(metabolite_names[i], {"expected_coef": 0.0})["expected_coef"] 
                           for i in range(n_features)])
    prior_sds = np.array([numerical_priors.get(metabolite_names[i], {"prior_sd": 1.0})["prior_sd"] 
                         for i in range(n_features)])
    
    with pm.Model() as model:
        # LLM-informed priors for coefficients (add regularization for stability)
        beta = pm.Normal("beta", mu=prior_means, sigma=np.minimum(prior_sds, 1.0), shape=n_features)
        ...
        with warnings.catch_warnings():
            ...
            idata = pm.sample(1000, tune=1000, cores=2, progressbar=False,
...
def fit_uninformative_bayesian_classifier(X_train, y_train, X_test):
    ...
    with pm.Model() as model:
        ...
        beta = pm.Normal("beta", mu=0, sigma=0.5, shape=n_features)  # Strong regularization
...
def run_cv_benchmark(abundance_data, group_labels, metabolite_names, sample_data, n_folds=5):
    ...
    try:
        ...
        print(f"Condition number: {cond_num:.2e}, using standardization + regularization")
    except:
        print("Could not compute condition number, using standardization + regularization")
    ...
    total_folds = n_folds
    pbar = tqdm(total=total_folds, desc="CV Benchmark", 
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y)):
        ...
        try:
            ...
            results.append({
                "fold": fold, "method": "LLM-Bayesian", 
                "accuracy": acc_llm, "auc": auc_llm
            })
            ...
            results.append({
                "fold": fold, "method": "Uninformative-Bayesian", 
                "accuracy": acc_uninf, "auc": auc_uninf
            })
            ...
            results.append({
                "fold": fold, "method": "Horseshoe", 
                "accuracy": acc_horse, "auc": auc_horse
            })
            
            # Ridge (handles its own preprocessing)
            y_pred_ridge, y_prob_ridge = fit_ridge_classifier(
            ...
            results.append({
                "fold": fold, "method": "Ridge", 
                "accuracy": acc_ridge, "auc": auc_ridge
            })
...
def create_predictive_visualizations(results_df):
    ...
    plt.savefig(f"{output_dir}/predictive_performance_benchmark.png", 
...
def main():
    ...
    abundance_data, group_labels, metabolite_names, sample_data = load_and_prepare_data()
