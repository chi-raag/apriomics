...
def analyze_llm_reasoning():
    ...
    print("="*60)
    print(f"Increases: {len(increases)} ({len(increases)/len(llm_predictions)*100:.1f}%)")
    print(f"Decreases: {len(decreases)} ({len(decreases)/len(llm_predictions)*100:.1f}%)")
    print(f"Unchanged: {len(unchanged)} ({len(unchanged)/len(llm_predictions)*100:.1f}%)")
    ...
    for entry in increases[:10]:  # First 10 increases
        ...
        print(f"   {entry['reasoning'][:200]}{'...' if len(entry['reasoning']) > 200 else ''}")
        
        # Extract key phrases
        diabetes_phrases = [
            'diabetes', 'diabetic', 'insulin resistance', 'glucose', 'hyperglycemia',
            'metabolic dysfunction', 'oxidative stress', 'inflammation'
        ]
    ...
    for entry in decreases:  # All decreases since there are fewer
        ...
        # Extract key phrases
        decrease_phrases = [
            'decreased', 'reduced', 'lower', 'depleted', 'clearance', 'excretion',
            'kidney function', 'renal', 'filtration'
        ]
    ...
    all_reasoning = ' '.join([pred.get('reasoning', '').lower() for pred in llm_predictions.values()])
    ...
    print(f"Increase confidence: μ={sum(increase_conf)/len(increase_conf):.3f}, range=[{min(increase_conf):.2f}, {max(increase_conf):.2f}]")
    print(f"Decrease confidence: μ={sum(decrease_conf)/len(decrease_conf):.3f}, range=[{min(decrease_conf):.2f}, {max(decrease_conf):.2f}]")
    ...
    print(f"1. Increase/Decrease ratio: {bias_ratio:.1f}:1 (should be closer to 2:1 or 3:1)")
    ...
    avg_reasoning_length = sum(len(pred.get('reasoning', '')) for pred in llm_predictions.values()) / len(llm_predictions)
