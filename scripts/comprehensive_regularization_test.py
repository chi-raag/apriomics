...
from sklearn.linear_model import ElasticNet, Lasso, Ridge as RidgeReg, LogisticRegression
...
def fit_llm_adaptive_lasso(X_train, y_train, X_test, metabolite_names, qualitative_predictions):
    ...
    llm_means = np.array([numerical_priors.get(metabolite_names[i], {"expected_coef": 0.0})["expected_coef"] 
                         for i in range(n_features)])
    llm_confidences = np.array([qualitative_predictions.get(metabolite_names[i], {"confidence": 0.5})["confidence"] 
                               for i in range(n_features)])
...
def fit_llm_informed_bayesian(X_train, y_train, X_test, metabolite_names, qualitative_predictions):
        ...
        try:
            if "LLM" in method_name:
                y_pred, y_prob, coefs = fit_func(X_train_proc, y_train, X_test_proc, 
                                                metabolite_names, qualitative_predictions)
            else:
                y_pred, y_prob, coefs = fit_func(X_train_proc, y_train, X_test_proc)
            ...
            print(f"  AUC: {auc:.3f}")
            print(f"  Sparsity: {sparsity:.3f} ({np.sum(np.abs(coefs) >= 0.01)}/{len(coefs)} non-zero)")
