...
from benchmark_prior_recovery import (
    load_mtbls1_data,
    calculate_ground_truth_log2fc,
    subsample_balanced,
    fit_uninformative_bayesian_baseline,
    fit_james_stein_shrinkage,
...
def test_empirical_bayes_methods():
    ...
    print(
        f"Subsample: {len(group_labels_subsample)} samples, {len(abundance_subsample.columns)} metabolites"
    )
    ...
    for method_name, fit_function in methods_to_test:
        ...
        try:
            ...
            if len(common_metabolites) == 0:
                print("  ❌ No common metabolites with ground truth")
                continue

            method_estimates = pd.Series(beta_estimates, index=metabolites)[
                common_metabolites
            ]
            ...
            if len(baseline) > 0:
                ...
                for _, row in results_df.iterrows():
                    if row["method"] != "Uninformative Bayesian":
                        ...
                        print(
                            f"    RMSE change: {rmse_diff:+.6f} ({'better' if rmse_diff < 0 else 'worse'})"
                        )
    else:
