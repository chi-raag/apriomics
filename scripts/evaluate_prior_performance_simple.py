...
def load_mtbls1_data():
    ...
    metadata_path = "/Users/chiraag/Projects/gwu/lab/apriomics/docs/examples/data/s_MTBLS1.txt"
    ...
    print(f"Loaded data: {abundance_data.shape[0]} samples, {abundance_data.shape[1]} metabolites")
...
"""
    ...
    if os.getenv("GOOGLE_API_KEY"):
        ...
        for name in metabolite_names:
            ...
            if any(keyword in name.lower() for keyword in ['glucose', 'creatinine', 'citrate', 'acetone']):
                expected_fc = np.random.choice([0.8, -0.8], p=[0.6, 0.4])  # Bias toward increase
...
def fit_bayesian_regression_model(X_train, y_train, priors_dict, metabolite_names):
    ...
    prior_precisions = np.array([1.0 / (priors_dict[name]['prior_sd']**2) for name in metabolite_names])
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kf.split(abundance_data)):
        ...
        for prior_name, priors_dict in prior_configs.items():
            ...
            try:
                ...
                model, scaler = fit_bayesian_regression_model(X_train, y_train, priors_dict, metabolite_names)
                ...
                print(f"    Accuracy: {accuracy:.3f}, AUC: {auc:.3f}, LogLik: {log_likelihood:.3f}")
...
def analyze_results(cv_results):
    ...
    for metric in metrics:
        ...
        for prior_type in prior_types:
            ...
            if col_name in cv_results.columns:
                values = cv_results[col_name].replace([np.inf, -np.inf], np.nan).dropna()
                if len(values) > 0:
    ...
    for metric in metrics:
        ...
        if all(col in cv_results.columns for col in [uniform_col, llm_col]):
            uniform_vals = cv_results[uniform_col].replace([np.inf, -np.inf], np.nan).dropna()
            ...
            if len(uniform_vals) > 0 and len(llm_vals) > 0:
                try:
                    ...
                    print(f"{metric.upper()}: LLM improvement = {improvement:+.3f}, t={stat:.3f}, p={pval:.4f}")
                except:
...
def create_visualizations(cv_results, summary_stats):
    ...
    plt.savefig(f'{output_dir}/prior_performance_simple.png', dpi=300, bbox_inches='tight')
    plt.show()
