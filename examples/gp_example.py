...
from chem_priors.priors import PriorData, get_smiles, generate_fingerprints, create_similarity_matrix, get_kernel, run_pipeline
...
def main():
    ...
    example_metabolites = [
        ...
        "Serine", "Threonine", "Proline", "Asparagine", "Glutamine"
    ]
    ...
    print(f"Found SMILES for {priors.smiles_data['smiles'].notna().sum()} out of {len(example_metabolites)} metabolites")
    
    # Only proceed with metabolites that have fingerprints
    metabolites_with_fps = priors.metabolite_names
    print(f"Using {len(metabolites_with_fps)} metabolites with fingerprints for modeling")
    ...
    plt.errorbar(
        range(n), predictions['mean'], 
        yerr=predictions['std'], 
        fmt='o', capsize=5, label='Predicted with std'
    )
